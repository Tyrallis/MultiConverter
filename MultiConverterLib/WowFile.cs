...
namespace MultiConverterLib
{
    public class WowFile
    {
        ...
        public string ReadString(int pos, int n)
        {
            return BitConverter.ToString(ReadBytes(pos, n));
        }
        public byte[] ReadBytes(int pos, int n)
        {
            return Data.ToList().Skip(pos).Take(n).ToArray();
        }
        ...
        public void WriteBytes(int pos, byte[] value)
        {
            Buffer.BlockCopy(value, 0, Data, pos, value.Length);
        }

        public void InsertBytes(byte[] values)
        {
            Array.Resize(ref Data, Data.Length + values.Length);
            Buffer.BlockCopy(values, 0, Data, Data.Length - values.Length, values.Length);
        }
